...
def convert_to_nightcore():
    ...
    params = {
        'speed': float(request.form.get('speed', 1.3)),
        'pitch': int(request.form.get('pitch', 4)),
        'volume': float(request.form.get('volume', 1.0)),
        'bass_boost': int(request.form.get('bass_boost', 0)),
        'treble': int(request.form.get('treble', 0)),
        'reverb': int(request.form.get('reverb', 0)),
        'echo': int(request.form.get('echo', 0))
    }
    ...
    try:
        ...
        # Convert the file with parameters
        logger.info("Starting nightcore conversion")
        converter = NightcoreConverter(input_path, **params)
